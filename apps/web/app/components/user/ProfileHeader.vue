...
// The full-width profile banner. Holds identity (avatar / name / role · status
// chips / 签名 / 私聊) plus the few HEADLINE metrics lifted out of the 关于 panel —
// the full stats stay in 关于. Structured as a banner so a user-customizable
// header background image can slot in later: pass `backgroundImage` and the
// solid tint is replaced by the image + a solid legibility scrim (no gradients —
// iron rule #2).
//
// On scroll, once the banner leaves the top, a slim sticky bar (small avatar +
// name + key stats) fades in below the top bar so the user's identity stays
// visible without the full banner's height. The banner itself scrolls away
// normally (the mini bar is `fixed`, so it adds no flow height → no scroll jump).
...
const hasBg = computed(() => !!props.backgroundImage)

// The headline metrics for the banner. The complete stats grid lives in 关于;
...
// Collapse when the full banner has scrolled above the top bar. rootMargin's
// -80px top inset ≈ the fixed top bar's height, so the mini bar appears exactly
// as the banner clears it. Client-only (VueUse no-ops on SSR) → starts expanded.
...
<template>
  <div>
    <section
      ref="bannerRef"
      :class="cn('relative overflow-hidden rounded-xl', !hasBg && 'bg-primary/5')"
    >
      <!-- Custom header background (future, user-customizable) + solid scrim for
           legibility. Solid overlay, never a gradient. -->
      <template v-if="hasBg">
        <img
          :src="backgroundImage"
          alt=""
      ...
      <div :class="cn('relative p-5 sm:p-6', hasBg && 'text-white')">
        <div class="flex items-start gap-4">
          ...
          <div class="min-w-0 flex-1">
            ...
            <p
              ...
              :class="
                cn(
                  'mt-2 line-clamp-2 text-sm break-words',
                  hasBg ? 'text-white/85' : 'text-default-600'
                )
              "
            >
              {{ user.bio }}
            ...
            <div
              :class="cn('mt-2 text-sm', hasBg ? 'text-white/70' : 'text-default-500')"
            >
        ...
        <div class="mt-5 flex flex-wrap gap-x-8 gap-y-3">
          <div v-for="m in metrics" :key="m.label" class="min-w-14">
            <div
              :class="cn('text-xl font-bold', m.accent && !hasBg && 'text-secondary')"
            >
              {{ m.value }}
            </div>
            <div
              :class="cn('text-xs', hasBg ? 'text-white/70' : 'text-default-500')"
            >
              {{ m.label }}
            </div>
          </div>
    ...
    <!-- Compact sticky bar: fades in once the banner scrolls above the top bar.
