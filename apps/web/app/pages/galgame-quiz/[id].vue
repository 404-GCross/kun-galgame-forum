...
const seoTitle = quiz
  ? `${maskSpoilers(quiz.question, '……').slice(0, 42)}${galgameNames ? `｜${galgameNames}` : ''}`
  : 'Galgame 答题'
...
const seoDescription = (() => {
  ...
  return `${prefix}${maskSpoilers(quiz.question, '……').slice(0, 70)} —— ${bits.join('，')}。来鲲 Galgame 论坛 Galgame 题库一起出题答题。`
})()
