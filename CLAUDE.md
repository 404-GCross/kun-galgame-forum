...
The authoritative contract docs are synced as **read-only mirrors** under `docs/{oauth,image_service,galgame_wiki,artifact}/` (the file headers carry a GENERATED banner).
...
- Modifying any file under `docs/{oauth,image_service,galgame_wiki,artifact}/` is **wrong** — those are infra's mirrors; to change them, change infra and then run `docs:sync`.
